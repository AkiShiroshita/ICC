...
$Y_{i,j,t} = \alpha + u_i + v_ij + \epsilon_{ijt}$ ($i$: patient, $j$: time, $t$: rater, $\alpha$: intercept)  
...
intra_boot_func <-  function(data, index){
  fit_intra <- lmer(angle ~ 1 + (1|time/id),
                    data = data[index,],
...
$Y_{i,j,t} = \alpha + u_i + v_{ij} + \epsilon_{ijk}$ ($i$: patient, $j$: rater, $k$: time, $\alpha$: intercept) 
...
inter_boot_func <-  function(data, index){
  fit_inter <- lmer(angle ~ 1 + (1|rater/id),
                    data = data[index,],
                    control=lmerControl(check.nobs.vs.nlev = "ignore",
                                        check.nobs.vs.rankZ = "ignore",
                                        check.nobs.vs.nRE="ignore"))
...
for (i in 1:length(datasets)) {
  res_inter2 <- boot(inter_boot_func2,
                  data = datasets[[i]],
                  R = r)
  inter_summary_results2[i,1] <- res_inter2[["t0"]]
  inter_summary_results2[i,c(2,3)] <- boot.ci(res_inter2,
                                             type = "perc")[["percent"]][c(4,5)]
}
```
